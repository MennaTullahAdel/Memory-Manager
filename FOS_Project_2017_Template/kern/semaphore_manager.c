...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	int check = get_semaphore_object_ID(ownerEnvID,semaphoreName);
		...
		{
			int all = allocate_semaphore_object(&SEM);
			if (all != E_NO_SEMAPHORE)
			{
				SEM[all].ownerID=ownerEnvID;
				strcpy(SEM[all].name,semaphoreName);
				SEM[all].empty=0;
			    SEM[all].value=initialValue;
				return all;
			}
			else
			{
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	int check = get_semaphore_object_ID(ownerEnvID,semaphoreName);
			if(check !=E_SEMAPHORE_NOT_EXISTS)
			{
				if (strcmp( semaphores[check].name , semaphoreName)==0)
				{
					semaphores [check].value--;
					if(semaphores[check].value < 0)
					{
						sched_remove_ready(curenv);
						enqueue(&semaphores[check].env_queue,curenv);
	...
	int check = get_semaphore_object_ID(ownerEnvID,semaphoreName);
				if(check !=E_SEMAPHORE_NOT_EXISTS)
				{
					...
					if(semaphores[check].value <=0 )
					{
						...
						myenv = dequeue(&semaphores[check].env_queue);
