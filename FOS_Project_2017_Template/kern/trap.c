...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	//refer to the project presentation and documentation for details
	if((unsigned int)env_page_ws_get_size(curenv) < curenv->page_WS_max_size)
	{
		...
		for(int i = 0; i < curenv->page_WS_max_size; i++)
		{
			if(env_page_ws_is_entry_empty(curenv, curenv->page_last_WS_index))
			{
				...
				if(map_frame(curenv->env_page_directory, fi, (void*)fault_va, PERM_PRESENT | PERM_USER | PERM_WRITEABLE) == E_NO_MEM)
				{
					return;
				}
				if (pf_read_env_page(curenv, (void*)fault_va) == E_PAGE_NOT_EXIST_IN_PF)
				{
					...
					{
						if (pf_add_empty_env_page(curenv, fault_va, 1) == E_NO_PAGE_FILE_SPACE)
						{
	...
	else
	{
		...
		if(pt_get_page_permissions(curenv, curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address) & PERM_MODIFIED)
		{
			//update
			uint32 *ptr_table = NULL;
			ptr_frame_info = get_frame_info(curenv->env_page_directory, (void*)curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address, &ptr_table);
			pf_update_env_page(curenv,(void *) curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address, ptr_frame_info);
		}
		//unmap
		...
		if(map_frame(curenv->env_page_directory, ptr_frame_info, (void*)fault_va, PERM_PRESENT | PERM_USER | PERM_WRITEABLE) == E_NO_MEM)
		{
			return;
		}
		if (pf_read_env_page(curenv, (void*)fault_va) == E_PAGE_NOT_EXIST_IN_PF)
