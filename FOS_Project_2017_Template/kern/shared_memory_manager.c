...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	// This function should create the shared object at the gi35112qwsxven virtual address with the given size
	...
	else
	{
		for(int i =0; i < size;i++)
		{
			...
			allocate_frame(&info);
			map_frame(myenv->env_page_directory,info,(void*)virtual_address,PERM_PRESENT|PERM_USER|PERM_WRITEABLE);
			add_frame_to_storage(shares[index].framesStorage,info,i);
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	{
			struct Frame_Info* info;
			info = get_frame_from_storage(shares [index].framesStorage, i);
			if(shares[index].isWritable == 1)
				{
					map_frame(myenv->env_page_directory,info,(void*)virtual_address,PERM_USER|PERM_WRITEABLE|PERM_PRESENT);

				//	cprintf("info %x \n address %x ",info,virtual_address);
				}
			else
				{
					map_frame(myenv->env_page_directory,info,(void*)virtual_address,PERM_USER|PERM_PRESENT);
				}
			//info -> references++;
			virtual_address += PAGE_SIZE;
	}
