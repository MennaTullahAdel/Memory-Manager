...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32 Physaddr = kheap_physical_address((unsigned int) ptr_page_table);
	ptr_page_directory[PDX(virtual_address)] = CONSTRUCT_ENTRY(Physaddr, PERM_WRITEABLE|PERM_USER|PERM_PRESENT);
	uint32* tmp = ptr_page_table;
...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	{
		pf_add_empty_env_page(e, (virtual_address + i), 1);
	}
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	for(uint32 i = 0; i < size; i += PAGE_SIZE)
	{
		...
		get_page_table(e->env_page_directory, (void*)(virtual_address + i), &PtrTable);
		if(PtrTable != NULL)
		{
			int isPageTableEmpty = 0;
			for(int m = 0; m < 1024; m++)
			{
				if((uint32)(PtrTable[m] & PERM_PRESENT) == 1)
				{
					isPageTableEmpty = 1;
					break;
				}
			}
			if(isPageTableEmpty == 0)
			{

				uint32 PA = kheap_physical_address((uint32)PtrTable);
